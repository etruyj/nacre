...
//          a new Pool. This allows for both NAS and DS3 pool creation
//          as the build logic is the same, the underlying API call is
//          the soul difference.
//
...
public class CreatePool {
    private static final Logger log = LoggerFactory.getLogger(CreatePool.class);
    ...
    public static Pool fromPoolConfig(PoolConfig pool, Pool defaults, BpConnector pearl) {
        ...
        System.err.println("Strip count: " + pool.getStripes());
        ...
        try {   
            ...
            new_pool.setUsed(0L); // set used capacity to 0
        ...
        log.debug("Successfully created pool [" + pool.getName() + "]");
        return new_pool;
    }

    public static Pool fromObject(Pool pool, BpConnector pearl) throws Exception {
        ...
        switch(pool.getType()) {
            ...
            case "Online":
            case "online":
                return createDs3Pool(pool, pearl);
            default:
                throw new Exception("Unable to process pool type [" + pool.getType() + "]");
        }
    
    }

    public static Pool createDs3Pool(Pool pool, BpConnector pearl) {
        log.info("Creating Ds3 pool [" + pool.getName() + "]");

        Pool new_pool = null;

        try {
            new_pool = pearl.createDs3Pool(pool);
    ...
    public static Pool createNasPool(Pool pool, BpConnector pearl) {
